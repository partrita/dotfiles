...
**Prevention:** Always explicitly disable modeline support in Vim (`set nomodeline` in `.vimrc`) and Neovim (`vim.opt.modeline = false` in `init.lua`) configurations to prevent any unintended code execution when opening files.
